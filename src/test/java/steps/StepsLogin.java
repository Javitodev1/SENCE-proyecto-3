...
public class StepsLogin {
    ...
    private enum Outcome { EXITO, FALLO }
...
public void validarResultadoExitoso(String esperado) {
    ...
    if ("exitoso".equalsIgnoreCase(esperado)) {
        ...
        if (real != Outcome.EXITO) {
            throw new AssertionError("Se esperaba EXITO pero fue " + real + ". URL actual: " + driver.getCurrentUrl());
        }
...
public void validarResultadoFallido(String esperado) {
    ...
    if ("fallido".equalsIgnoreCase(esperado)) {
        if (real != Outcome.FALLO) {
            throw new AssertionError("Se esperaba FALLO pero fue " + real + ". URL actual: " + driver.getCurrentUrl());
        }
        ...
        } catch (NoSuchElementException ignored) { /* Ok */ }
    }
}

/** Espera única: o redirige a /profile (éxito) o aparece #name con "Invalid..." (fallo). Cachea el resultado. */
private Outcome detectarResultado(Duration timeout) {
    if (ultimoResultado != null) return ultimoResultado;
    ...
    try {
        ...
        ultimoResultado = w.until(d -> {
            // ¿Éxito?
            if (d.getCurrentUrl().contains("/profile")) return Outcome.EXITO;
